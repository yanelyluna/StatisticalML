...
y_est <- summary(modeloBos_multiple)$coefficients[-c(1,seq(9,15)),1] #solo comparamos los b1 de 
#niveles de referencia de las variables categórica
x_est_log <- resumen_log$beta_1
y_est_log <- summary(modeloBos_multiple_log)$coefficients[-c(1,seq(9,15)),1] 
par(mfrow = c(1,1))
...
kable(resumen_nl)
par(mfrow=c(2,4))
#for (i in 1:length(modelos_nl)) {
# plot(x = augment(modelos_nl[[i]])$Boston...i.,y=Boston$crim, xlab = namesBos[numBos[i]], ylab = "crim rate", main = paste0("crim ~ ", namesBos[numBos[i]]),cex = 5, pch = '.', col = "#7d3e79") 
#lines(x=distinct(augment(modelos_nl[[i]]),Boston...i.),y=distinct(augment(modelos_nl[[i]]),Boston...i.,.fitted), col="blue",type = "l")
#}
#for (i in 1:length(modelos_nl)) {
...
levels(june_13$Rd_Feature)[c(1,3)]="INTERSECTION_DRIVEWAY_NONE"
modelos_jun13[[4]]<- lm(log(Crash_Score)~.,data = june_13);summary(modelos_jun13[[4]])
levels(june_13$Rd_Feature)[c(4,5)]="OTHER_RAMP"
levels(june_13$Rd_Configuration)[c(1,3,5)]="ONE-WAY_TWO-WAY-PROTECTED_UNKNOWN"
modelos_jun13[[4]] <- lm(log(Crash_Score) ~.,data = june_13);summary(modelos_jun13[[4]])
