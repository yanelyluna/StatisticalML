...
modelprob<-predict(model,datos1[,1:7], type = "raw"); modelprob
...
par(mfrow=c(1,1))
plot(Matriz_confusion, color="purple", main="Matriz confusiÃ³n Naive Bayes")
...
###Training Test 0.75*532
##400 observaciones training
##132 observaciones test
set.seed(2346)
training_nb <- createDataPartition(datos1$type,p=0.75)$Resample1
#0    1 
#267 133 
table(datos1$type[training_nb])
model_tr <- naiveBayes(type~ ., datos1, subset=training_nb); model_tr
model_pred <- predict(model_tr,datos1[-training_nb,])
tasas_error(type[-training_nb],model_pred)
#                 Global     0     1
#Aparentes NaiveB  23.87 18.59 34.46
#Training B=1000   23.09 17.66 33.98
#Test B=1000       23.78 18.34 34.68
B=1000
err_test.training=matrix(NA,ncol=3, nrow=1000)
colnames(err_test.training)=c("global.tst","No.tst","Yes.tst")
set.seed(1120)
for (i in 1:1000) {
  train <- sample(532,426)
...
fit_rad = tune(svm , type~., data = datos1, kernel ="radial",
               ranges = list(cost=c(1:5, 2^(2:9)),gamma =c(0.5 ,1 ,2 ,3 ,4) ))
summary(fit_rad)
## Mejor cost=1      gamma=0.5 menor error=0.2912 
# Mejores modelos
...
#Contruimos la matriz de errores del modelo seleccionado
errores_svm=matrix(NA, ncol=3, nrow=2)
colnames(errores_svm)=c("Global","Grupo 0", "Grupo 1")
rownames(errores_svm)=c("Aparentes K-Radial",
                        "Test K-Rad Tr/Tst B=500")

taux=table(type, predict(fit_rad$best.model, datos1[ ,-8]))
errores_svm[1,1]=(1- sum(diag(taux))/sum(taux))*100
errores_svm[1,2]=(1- taux[1,1]/sum(taux[1,]))*100
errores_svm[1,3]=(1- taux[2,2]/sum(taux[2,]))*100
...
#                         Global Grupo 0 Grupo 1
#Aparentes K-Radial        9.586   3.944  20.90
#Test K-Rad Tr/Tst B=500   9.589   4.069   20.82
#####
set.seed(1234)
B=500
...
for(irep in 1:B){
  ...
  Mbest=fit_rad$best.model
  prms[irep,1]=fit_rad$best.parameters[[1]]
  prms[irep,2]=fit_rad$best.parameters[[2]]
  tauxx=table(type[-train], predict(Mbest, datos1[-train,]))
...
errores_svm[2,2]=mean(errs[,1])*100
errores_svm[2,3]=mean(errs[,2])*100
...
errores=matrix(NA, ncol=3, nrow=3)
...
par(mfrow=c(1,1))
matplot(1:3,cbind(100*errores[1,],100*errores[2,],100*errores[3,]),pch=19, cex=1,
